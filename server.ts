...
async function startServer() {
  ...
  app.post('/api/rooms', requireOwnerOrAdmin, (req, res) => {
    try {
      ...
      db.run(
        `INSERT INTO rooms (id, title, description, price, location, city, room_type, status, photos, amenities, owner_id, contact_name, contact_phone, contact_email, contact_whatsapp, views_count, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?);`,
        [
          newRoomId,
          cleanTitle,
    ...
    try {
      ...
      db.run(
        `UPDATE rooms SET 
          ...
          photos = ?, amenities = ?, contact_name = ?, contact_phone = ?, contact_email = ?, contact_whatsapp = ?, updated_at = ?
         WHERE id = ?;`,
        [
          cleanTitle,
          cleanDesc,
